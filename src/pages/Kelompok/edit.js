...
const Edit = ({navigation, route}) => {
    ...
    const handleSubmitForm = async (values) => {
        ...
        const { status, message, data } = await axiosPost({dispatch, route: 'kelompok/edit',
            headers: {
                token: user.token
            },
            data: { ...values, id_kelompok: route.params.id_kelompok},
        })
    ...
    return (
        <View style={{ flex: 1, backgroundColor: 'white'}}>
            ...
            />
            <Formik
                initialValues={{ nama: route.params.nama }}
