...
const ToastError = () => {
  ...
  useEffect(() => {
    ...
    if (toast.show && toast.text && toast.type == 'error') {
      return (
          <Animated.View style={{ ...styles.loaderContainer, opacity: animOpacity, transform: [{
              translateY: heightToast
          }]}}>
              <View style={{ ...styles.cardToast, backgroundColor: colors.error }}>
              ...
              <TouchableWithoutFeedback onPress={() => handleOnPress({toast, dispatch, setToast})}>
                  <Text style={{ ...styles.textAksi, color: colors.error }}>{toast.textAksi}</Text>
